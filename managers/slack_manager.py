    ...
    def handle_slack_exception(self, slack_error_msg):
        if slack_error_msg == "not_authed":
            ...
            raise SlackManagerError(
                "The channel to which the message was requested to be sent to does not exist",
                error_code=400,
            )
        else:
            print("Unhandled exception ocurred: ", slack_error_msg)
            raise SlackManagerError(
                f"Unknown error ocurred, please contact the administrator. Reference error message {slack_error_msg}.",
                error_code=500,
            )
